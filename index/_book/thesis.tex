...
\Acknowledgements{
\emph{To \textbf{Kelly}, my adviser: Thank you for not only your patience and countless hours of support in and out of thesis meetings, but also for your mentorship in regards to so many aspects of my life in the last year. I couldn't have asked for a more caring or thoughtful adviser, you made this project so enjoyable.}
\newline
\newline
\emph{To \textbf{Gretchen Moisen}: thank you for your consistent support of this project and all of your forestry-related insight. It has been a great time getting to know you over this year.}
\newline
\newline
\emph{To \textbf{Brittney}, \textbf{Thomas}, \textbf{Elliot}, and \textbf{Theo}: I am so grateful to have spent the last four years living in the same city as you all. Thank you for the countless meals and all the love and support you have given me. Brittney, thank you for your contributions in editing this thesis, my first HUM paper, and just about everything in between. I love you all so much.}
\newline
\newline
\emph{To \textbf{Simon (and Millie)}: thank you for being my best friend. I am so glad we were able to spend a great deal of this year together and share our thesis experiences despite the pandemic. I can't wait to be spending time in Baltimore and wherever else is next for you.}
\newline
\newline
\emph{To \textbf{Maurvie}: thank you for being such an amazing friend and for spending so much great time over the last few years. Your significant contribution to the creation of the color palette used in this thesis brought my plots to a whole new level. I hope to find many more co-ops in the future with you.}
\newline
\newline
\emph{To \textbf{Josh Y}: thank you for your genuine interest in this thesis and the friendship gained from working on an adjacent project. It's been so great to get to know you well this year, keep in touch.}
\newline
\newline
\newline
\emph{To \textbf{Ben} and \textbf{Emma}: thank you for sharing your house with me for the last bit of this year. It's been so great to get closer to you both, and I hope we stay in touch after Reed.}
\newline
\newline
\emph{To \textbf{Shisham}: the time spent collaborating on assignments through the years has been so much fun. I'm so excited to visit you in Davis!}
\newline
\newline
\emph{To \textbf{Reed College Basketball}: thank you for being my community and a place that felt like home at Reed. I hope in the not-too-distant future Sunday ball will return and we will get to be together again.}
\newline
\newline
\emph{To my sisters, \textbf{Erin} and \textbf{Willow}: you mean the world to me, and I love you both so much. Thanks for all the phone calls and for visiting me as much as possible.}
\newline
\newline
\emph{To \textbf{Kayla}: I likely wouldn't have made it through Reed without those first couple years with you. Nor would I be the person I am today, I hope you're doing well.}
\newline
\newline
\emph{To \textbf{Phyllis and Bob Henigson}: Thank you for your generosity in supporting and significantly contributing to funding my education over the past four years. Your contributions have made my education possible.}
\newline
\newline
\emph{To \textbf{Dude}, the good boy: thank you for putting up with me while I sat at this desk for hours on end working on this thesis. I couldn't have asked for a better companion.}
}

\Dedication{
\emph{To my parents, Greg and Tess. I wouldn't be here today without you both, and I couldn't have grown up in anything more special than that 14' by 24' cabin in the middle of the forest. Thank you for everything.}
}
...
\tightlist
\item
  The remainder of Chapter \ref{context} discusses current common approaches for estimation of forest attributes. I also introduce less common approaches to performing this estimation, such as the hierarchical Bayesian approach. I introduce the idea of borrowing strength and discuss how different estimators borrow strength.
\item
  Chapter \ref{data} introduces the data used to accomplish the research goal. I discuss the nested structure of the data and how these data are appropriate for hierarchical modeling. I explore the spatial aspects of the response variables and the distributions of these variables. I also introduce the data used for the explanatory variable and the data used for strata weighting.
\item
  Chapter \ref{methods} builds up the theory and mathematics behind the estimators used in this thesis. Starting at the simplest case of the sample mean, I progressively introduce new estimators which take varying levels of information into account. I also introduce some theory regarding Bayesian statistics in order to build intuition for the hierarchical Bayesian estimators.
\item
...
The FIA has been continuously operating since 1930, and its official mission is to ``make and keep current a comprehensive inventory and analysis of the present and prospective conditions of and requirements for the renewable resources of the forest and rangelands of the US.'' (FIA, 2020). The FIA collects data all throughout the United States by completing a survey each year of many plots of land. The areas measured by the FIA crews in the field are comprised of 4 subplots, distributed within a .61 hectare circle. Due to the vast size of the United States and immense amount of forested land, it would be nearly impossible for the FIA to attain population data for the entire country, so it uses sampling instead. The FIA samples from the population of plots by using a geographically-based systematic sampling design (Bechtold \& Patterson, 2005). The FIA chooses these samples by first overlaying a hexagonal grid over the United States in which each hexagon contains approximately 6000 acres of land. It then fills these hexagons with plots and randomly samples from these plots. Next, ground crews go to these sampled plots and collect variables such as basal area, trees per acre, etc.

Along with this hand-collected data from ground crews, the FIA also uses remotely-sensed data to gain more information. For example, the \texttt{nlcd11} variable, which predicts total percent tree canopy cover over each 30 meter pixel, is collected via remote sensing by the Multi-Resolution Land Characteristics Consortium (Homer, 2015). Throughout the duration of the thesis, I will be working to predict ground-collected data with remotely-sensed variables, such as \texttt{nlcd11}. Having remotely-sensed variables like \texttt{nlcd11} is useful to the FIA because if these variables can predict ground-collected variables well, the FIA can collect less data and have a larger effective sample size. The \texttt{nlcd11} variable is particularly useful as the data is available at a very fine (pixel-level) resolution across the entire Interior West.
...
\end{figure}
The data cover a total of 14 eco-provinces, 68 eco-sections, and 480 eco-subsections. The hierarchical structure of the data and nested nature of the eco-subsections within eco-sections within eco-provinces allows for the creation of hierarchical models that borrow strength from surrounding areas.
...
The most intuitive approach to estimating the population mean \(\mu_{y_j}\) might be the sample mean, i.e.~using the ``Horvitz-Thompson estimator,'' as it is referred to by survey samplers (Horvitz \& Thompson, 1952). This estimator can be expressed as follows:
\begin{align}
...
The post-stratified estimator is very similar to the Horvitz-Thompson estimator; however, as stated above, it addresses variance that occurs from using the Horvitz-Thompson estimator. While decreasing variance seems like a no-cost solution to some of the problems with the Horvitz-Thompson estimator, the post-stratified estimator requires auxiliary information in order to be used. The post-stratified estimator is a weighted sum of two Horvitz-Thompson estimators: one Horvitz-Thompson estimator giving the estimate of the mean in sampled units that are forested and the other Horvitz-Thompson estimator in non-forested areas. Next, the post-stratified estimator weights these estimates by the predicted proportion of area in the eco-subsection of interest that is forested. The FIA uses these weights to account for non-representation in sampling due to non-response, but this non-response is not explored in this thesis. Note that while the post-stratified estimator uses auxiliary information such as the predicted proportion of forested area in the eco-subsection of interest and whether or not the sampled units were forested areas or not, both of these pieces of information only consider the eco-subsection of interest. Therefore, since information is not used from outside of the eco-subsection of interest, the post-stratified estimator is still within the family of direct estimators. The post-stratified estimator is represented as follows:
\begin{align}
...
\end{align}
Recall that \(k\) indexes over the strata, which in this case is forested and non-forested sampled units. The quantity \(w_{jk}\) is a survey weight entirely decided by the predicted proportion of forested area in eco-subsection \(j\). For example, if eco-subsection \(j\) was 80\% forested, the weights would be calculated as \(w_{j1} = 0.8\) and \(w_{j2} = 0.2\). Therefore if under-sampling of forests had occurred (say, if only 60\% of our samples were forested), this under-sampling is corrected with the survey weights, and this results in a better estimate of \(\mu_{y_j}\) that is less variable.
...
\end{align}
Note that I use capital \(X\) to denote the population value of the explanatory variable at the unit-level. The unit-level regression-synthetic estimator is expressed below:
\begin{align}
...
\begin{align}
P(\mu_{y_j} ~\vert~ \text{data}) \propto P(\text{data} ~\vert~ \mu_{y_j}) \cdot P(\mu_{y_j})
\label{eq:bayes-thm}
\end{align}
...
\end{align*}
where \(m\) is the number of small areas in the eco-province where the estimator is fit. Now that the estimator has been specified, it is easy to attain the small area estimate and variance. For the estimate in eco-subsection \(j\), the Bayes estimator for \(\mu_{y_j}\) is used:
\begin{align}
...
\leavevmode\vadjust pre{\hypertarget{ref-eco-sub-pic}{}}%
Cleland, D. T., Freeouf, J. A., Keys, J. E., Nowacki, G. J., Carpenter, C. A., \& McNab, W. H. (2007). Ecological subregions: Sections and subsections for the conterminous united states. Retrieved from \url{https://www.fs.fed.us/research/publications/misc/73326-wo-gtr-76d-cleland2007.pdf}
