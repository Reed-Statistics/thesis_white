...
Currently, there are three main types of estimators used to estimate the value of forest attributes: direct, indirect with implicit models, and indirect with explicit models.. Direct estimators are commonly thought of as the simplest estimators as they do not borrow strength nor do they use auxiliary variables for estimation. Direct estimators are hence easy to use and interpret, but we often do not get precise enough estimates with these estimators. Indirect and model-based estimators, such as the EBLUP estimator, aim to reduce variance in estimates by using data from surrounding areas and (in the case of the model-based estimator) using auxiliary data.
