...
We used the R statistical software to compute our results (R Core Team, 2020). The sample mean was computed using the \emph{tidyverse} (Wickham et al., 2019). The post-stratified estimates were computed using \emph{mase} (McConville, Tang, Zhu, Cheung, \& Li, 2018). The frequentist EBLUP estimates were computed using \emph{sae} (Molina \& Marhuenda, 2015). The hierarchical Bayesian estimates were computed using \emph{hbsae} (Boonstra, 2012). Our data spans the entire Interior West, however we were forced to exclude a few eco-provinces. Namely two eco-provinces with a very small number of eco-subsections which caused errors in producing the unit-level hierarchical Bayesian estimates and three eco-provinces which caused errors in producing the area-level hierarchical Bayesian estimates. While this is disappointing, we still were able to fit these estimators to much data.

This thesis uses all six of these estimators to produce estimates for basal area (square-foot), tree count per acre, above-ground biomass (lbs), and net volume (\(\text{ft}^3\)). The EBLUP and hierarchical Bayesian estimators use one explanatory variable, total canopy cover, to produce estimates. Our estimation occurs at the eco-subsection level, and thus we have 7,128 estimates produced (six estimators, four response variables, and 297 eco-subsections).

We are generally concerned with producing estimates that have both low variance and low bias. So, this chapter primarily aims to answers questions regarding these two quantities. In order to do so, we will first summarize some findings with a case study in the Northern Rocky Forest. This case study allows us to dig in to results without much aggregation or overwhelming plots due to the large amount of results we have. Next, we will explore variation measures and visualizations across the interior west in order to quantify variance of our estimators. Finally, we will attempt to quantify bias of estimators across the interior west.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{res_long }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(province }\OperatorTok{==}\StringTok{ "M333"}\NormalTok{,}
\NormalTok{         response }\OperatorTok{==}\StringTok{ "BALIVE_TPA"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{subsection =} \KeywordTok{str_sub}\NormalTok{(subsection, }\DecValTok{5}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\DataTypeTok{mapping =} \KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ subsection,}
                       \DataTypeTok{y =}\NormalTok{ cov,}
                       \DataTypeTok{color =}\NormalTok{ estimator)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_manual}\NormalTok{(}\DataTypeTok{values =} \KeywordTok{c}\NormalTok{(}\StringTok{"#A9A9A9"}\NormalTok{, }\StringTok{"#334F44"}\NormalTok{, }\StringTok{"#E7AB26"}\NormalTok{,}
                                \StringTok{"#BF4C27"}\NormalTok{,  }\StringTok{"steelblue"}\NormalTok{,  }\StringTok{"#80BCA2"}\NormalTok{),}
                     \DataTypeTok{labels =} \KeywordTok{c}\NormalTok{(}\StringTok{"Sample Mean"}\NormalTok{, }\StringTok{"Post-Stratification"}\NormalTok{, }\StringTok{"Area EBLUP"}\NormalTok{,}
                                \StringTok{"Unit EBLUP"}\NormalTok{, }\StringTok{"Area HB"}\NormalTok{, }\StringTok{"Unit HB"}\NormalTok{)) }\OperatorTok{+}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{means <-}\StringTok{ }\NormalTok{res_long }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(estimator }\OperatorTok{==}\StringTok{ "dirmean"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{rename}\NormalTok{(}\DataTypeTok{mean =}\NormalTok{ estimate) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{select}\NormalTok{(subsection, response, mean)}
\NormalTok{res_long <-}\StringTok{ }\NormalTok{res_long }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{full_join}\NormalTok{(means, }\DataTypeTok{by =} \KeywordTok{c}\NormalTok{(}\StringTok{"subsection"}\NormalTok{ =}\StringTok{ "subsection"}\NormalTok{,}
                          \StringTok{"response"}\NormalTok{ =}\StringTok{ "response"}\NormalTok{))}
\NormalTok{res_long <-}\StringTok{ }\NormalTok{res_long }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{se =}\NormalTok{ mean }\OperatorTok{*}\StringTok{ }\NormalTok{cov)}
...
\NormalTok{         response }\OperatorTok{==}\StringTok{ "BALIVE_TPA"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{subsection =} \KeywordTok{str_sub}\NormalTok{(subsection, }\DecValTok{5}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(estimator }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"dirps"}\NormalTok{, }\StringTok{"hb_area"}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\DataTypeTok{mapping =} \KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ subsection,}
                       \DataTypeTok{y =}\NormalTok{ estimate,}
                       \DataTypeTok{color =}\NormalTok{ estimator)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_errorbar}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{ymin =}\NormalTok{ estimate }\OperatorTok{-}\StringTok{ }\NormalTok{se, }\DataTypeTok{ymax =}\NormalTok{ estimate }\OperatorTok{+}\StringTok{ }\NormalTok{se)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_manual}\NormalTok{(}\DataTypeTok{values =} \KeywordTok{c}\NormalTok{(}\StringTok{"#334F44"}\NormalTok{, }\StringTok{"steelblue"}\NormalTok{),}
                     \DataTypeTok{labels =} \KeywordTok{c}\NormalTok{(}\StringTok{"Post-Stratification"}\NormalTok{, }\StringTok{"Area HB"}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{theme_bw}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{legend.position =} \StringTok{"bottom"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-14-2.pdf}
...
\bottomrule
\end{longtable}
From Figure \ref{fig:cov-violin}, it is clear that the distribution of coefficient of variation for the area-level hierarchical Bayesian model is much lower than the others with a value of 0.046. When we compare this value to the post-stratification median coefficient of variation (0.137) and the area-level EBLUP median coefficient of variation (0.114) we see an extremely large increase in variation in each case.
