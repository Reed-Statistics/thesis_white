...
```{r fig.align="center", out.width='100%', echo = FALSE, warning = F, message = F, fig.cap = "The hierarchical Bayesian area-level estimator and post-stratified estimator predicting basal area in the Northern Rocky Forest.", fig.scap = "HB Area and Post-stratified estimates in M333"}
res_long %>%
...
```{r fig.align="center", out.width='100%', echo = FALSE, warning = F, message = F, fig.cap = "The frequentist area-level estimator and post-stratified estimator predicting basal area in the Northern Rocky Forest.", fig.scap = "EBLUP area-level and Post-stratified estimates in M333"}
res_long %>%
  filter(province == "M333",
         response == "BALIVE_TPA") %>%
  mutate(subsection = str_sub(subsection, 5),
...
p2 <- res_sf %>%
  ...
  theme(legend.position = "none") 
p3 <- res_sf %>%
  mutate(hb_area_l_freq_area = cov_hb_area < cov_freq_unit) %>%
  ggplot(mapping = aes(fill = hb_area_l_freq_area,
                       geometry = geometry)) +
  geom_sf(color = "grey30") +
  scale_fill_manual(values = c("#BF4C27", "#80BCA2"), na.value = "grey") +
  theme_void() +
  labs(title = "Area HB CV < Unit EBLUP CV") +
  theme(legend.position = "none")

p4 <- res_sf %>%
  mutate(hb_area_l_freq_area = cov_hb_unit < cov_dirps) %>%
  ggplot(mapping = aes(fill = hb_area_l_freq_area,
                       geometry = geometry)) +
  geom_sf(color = "grey30") +
  scale_fill_manual(values = c("#BF4C27", "#80BCA2"), na.value = "grey") +
...
p5 <- res_sf %>%
  mutate(hb_area_l_freq_area = cov_hb_unit < cov_freq_area) %>%
  ggplot(mapping = aes(fill = hb_area_l_freq_area,
                       geometry = geometry)) +
  geom_sf(color = "grey30") +
  scale_fill_manual(values = c("#BF4C27", "#80BCA2"), na.value = "grey") +
  theme_void() +
  labs(title = "Unit HB CV < Area EBLUP CV",
       fill = "") +
  theme(legend.position = "bottom",
