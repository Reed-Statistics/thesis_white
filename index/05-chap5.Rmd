...
We used the R statistical software to compute our results [@r-software]. The sample mean was computed using the *tidyverse* [@tidyverse]. The post-stratified estimates were computed using *mase* [@mase]. The frequentist EBLUP estimates were computed using *sae* [@sae-package]. The hierarchical Bayesian estimates were computed using *hbsae* [@hbsae-package]. Our data spans the entire Interior West, however we were forced to exclude a few eco-provinces. Namely two eco-provinces with a very small number of eco-subsections which caused errors in producing the unit-level hierarchical Bayesian estimates and three eco-provinces which caused errors in producing the area-level hierarchical Bayesian estimates. While this is disappointing, we still were able to fit these estimators to much data. 
...
## Basal Area in the Northern Rocky Forest: A Case Study
...
means <- res_long %>%
  ...
  select(subsection, response, mean)
res_long <- res_long %>%
