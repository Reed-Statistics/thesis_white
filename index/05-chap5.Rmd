...
We can see this phenomenon below by examining the performance of the hierarchical Bayesian area-level estimator compared to the post-stratified estimator: 
...
res_sf %>%
  ...
  theme_void() +
  labs(fill = "Area HB CV < Post-Strat CV") +
  theme(legend.position = "bottom") +
...
res_sf %>%
  ...
  scale_fill_manual(values = c("#BF4C27", "#80BCA2"), na.value = "grey") +
  theme_void() +
  labs(fill = "Area HB CV < Area EBLUP CV") +
  theme(legend.position = "bottom") +
  guides(fill = guide_legend(title.position = "top", title.hjust = 0.5))
```
