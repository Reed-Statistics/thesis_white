...
library(gt)
dat_small %>%
  select(INVYR, PLOT, LON_PUBLIC, LAT_PUBLIC, ELEV_PUBLIC, forgrp, forprob, demLF) %>%
  slice(30:35) %>%
  gt()

dat_small %>%
  select(forbio, BALIVE_TPA, CNTLIVE_TPA, BIOLIVE_TPA, VOLNLIVE_TPA, subsection, section, province) %>%
  slice(30:35) %>%
  gt()
```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, cache = TRUE, fig.retina = 4, fig.align = 'center', fig.height=4, fig.width=6}
...
```{r, message = FALSE, warning=FALSE, echo = FALSE, fig.align="center", fig.retina=4, fig.height=4, fig.width=6, cache=TRUE}
