...
## How many subsections are in sections? Sections in provinces?
...
### Exploring key variables variation at different levels
...
```{r}
comparison_df <- cbind(comparison, comparison_prov) %>%
  select(1:3, 5) %>%
  relocate(variable_name, overall_sd_of_mean, sd_of_mean_by_province) %>%
  rename(variable = variable_name,
         overall_variation = overall_sd_of_mean,
...
comparison_props <- comparison_df %>%
  mutate(
    province_overall = variation_by_province / overall_variation,
    section_overall = variation_by_section / overall_variation,
    overall_normalized = rep(1, 5)
  ) %>%
  select(province_overall, section_overall, overall_normalized, variable)

comparison_pivot <- comparison_props %>%
  pivot_longer(names_to = "variation", values_to = "value", cols = c(overall_normalized,
                                                              province_overall,
                                                              section_overall
                                                              ))
...
comparison_pivot %>%
  mutate(
    Level = case_when(
      variation == "overall_normalized" ~ "Overall",
      variation == "province_overall" ~ "Province",
      variation == "section_overall" ~ "Section"
    )
  ) %>%
  ...
  theme_bw() +
  scale_fill_manual(values = c("darkgreen", "forestgreen", "lightgreen")) +
  labs(
    title = "Proportion of Variation of Forest Attributes By Province and Section",
    subtitle = "Normalized by overall variation in the Interior West",
