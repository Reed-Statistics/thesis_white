...
cor(Xpop$mean, model_df$estimates) # Bad

model_df$estimates / Xpop$mean
```


```{r}
## try to do the area level stuff

# pop_data <- north_rocky %>%
#   group_by(subsection) %>%
#   summarize(forprob = sum(forprob),
#             `(Intercept)` = 1)
# 
# sae0 <- fSAE(BIOLIVE_TPA ~ forprob,
#              data = north_rocky,
#              area = "subsection",
#              type = "direct",
#              #popdata = pop_data,
