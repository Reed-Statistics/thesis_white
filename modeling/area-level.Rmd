...
mod_area <- stan_lmer(mean_BIOLIVE ~ mean_nlcd11 + 1 + (1 | section),
          data = m333_area)
mod_area_lm <- lme4::lmer(mean_BIOLIVE ~ mean_nlcd11 + (1 | section), data = m333_area)
...
MSE_direct <- (m333_area %>% # I am not sure if this is how the calc the direct MSE
  group_by(section) %>%
...
aparant_ss <- (MSE_direct / MSE_bayes) * nrow(m333_area)
aparant_ss
```
