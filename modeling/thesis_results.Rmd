...
intwest_no0 <- intwest %>%
  ...
  filter(!(province %in% c("M261", "M334", # two eco-subs each, divergent integrals
...
results <- data.frame(
  ...
  response = c(
    ...
    rep("BALIVE_TPA", 297),
    rep("CNTLIVE_TPA", 297),
    rep("VOLNLIVE_TPA", 297))
)
```

```{r mean}
dirmean <- list()
dirmean[1:9] <- lapply(iw, direct_estimate, response = "BIOLIVE_TPA", "subsection")
dirmean[10:18] <- lapply(iw, direct_estimate, response = "BALIVE_TPA", "subsection")
dirmean[19:27] <- lapply(iw, direct_estimate, response = "CNTLIVE_TPA", "subsection")
dirmean[28:36] <- lapply(iw, direct_estimate, response = "VOLNLIVE_TPA", "subsection")
...
set.seed(1)
bayes_unit <- list()
bayes_unit[1:9] <- lapply(iw, hb_unit, formula = BIOLIVE_TPA ~ nlcd11, small_area = "subsection", pop_data = tccpop)
bayes_unit[10:18] <- lapply(iw, hb_unit, formula = BALIVE_TPA ~ nlcd11, small_area = "subsection", pop_data = tccpop)
bayes_unit[19:27] <- lapply(iw, hb_unit, formula = CNTLIVE_TPA ~ nlcd11, small_area = "subsection", pop_data = tccpop)
bayes_unit[28:36] <- lapply(iw, hb_unit, formula = VOLNLIVE_TPA ~ nlcd11, small_area = "subsection", pop_data = tccpop)

# saveRDS(bayes_unit, "bayes_unit.rds")

res_hbu <- data.frame(
  subsection = names(unlist(lapply(bayes_unit, EST))),
  response = c(rep("BIOLIVE_TPA", 297),
               rep("BALIVE_TPA", 297),
               rep("CNTLIVE_TPA", 297), 
               rep("VOLNLIVE_TPA", 297)),
...
set.seed(1)
bayes_area <- list()
bayes_area[1:9] <- lapply(iw, hb_area, formula = BIOLIVE_TPA ~ nlcd11, small_area = "subsection", pop_data = tccpop)
bayes_area[10:18] <- lapply(iw, hb_area, formula = BALIVE_TPA ~ nlcd11, small_area = "subsection", pop_data = tccpop)
bayes_area[19:27] <- lapply(iw, hb_area, formula = CNTLIVE_TPA ~ nlcd11, small_area = "subsection", pop_data = tccpop)
bayes_area[28:36] <- lapply(iw, hb_area, formula = VOLNLIVE_TPA ~ nlcd11, small_area = "subsection", pop_data = tccpop)
...
set.seed(1)
frequnit <- list()
frequnit[1:9] <- lapply(iw, freq_unit, formula = BIOLIVE_TPA ~ nlcd11, "subsection", tccpop)
frequnit[10:18] <- lapply(iw, freq_unit, formula = BALIVE_TPA ~ nlcd11, "subsection", tccpop)
frequnit[19:27] <- lapply(iw, freq_unit, formula = CNTLIVE_TPA ~ nlcd11, "subsection", tccpop)
frequnit[28:36] <- lapply(iw, freq_unit, formula = VOLNLIVE_TPA ~ nlcd11, "subsection", tccpop)
...
set.seed(1)
freqarea <- list()
freqarea[1:9] <- lapply(iw, freq_area, formula = BIOLIVE_TPA ~ nlcd11, "subsection", tccpop)
freqarea[10:18] <- lapply(iw, freq_area, formula = BALIVE_TPA ~ nlcd11, "subsection", tccpop)
freqarea[19:27] <- lapply(iw, freq_area, formula = CNTLIVE_TPA ~ nlcd11, "subsection", tccpop)
freqarea[28:36] <- lapply(iw, freq_area, formula = VOLNLIVE_TPA ~ nlcd11, "subsection", tccpop)
...
post_strat_bio_rs_df <- bind_rows(post_strat_bio_rs) %>%
  ...
  mutate(rep = unlist(lapply(1:500, rep, 297))) %>%
  mutate(response = rep("BIOLIVE_TPA", 500 * 297),
         var = sd^2,
...
post_strat_ba_rs_df <- bind_rows(post_strat_ba_rs) %>%
  mutate(subsection = rep(names(subsection_list), 500)) %>%
  mutate(rep = unlist(lapply(1:500, rep, 297))) %>%
  mutate(response = rep("BALIVE_TPA", 500 * 297),
         var = sd^2,
         section = str_remove_all(subsection, "[:lower:]"),
         province = str_sub(section, end = -2))
...
post_strat_cnt_rs_df <- bind_rows(post_strat_cnt_rs) %>%
  mutate(subsection = rep(names(subsection_list), 500)) %>%
  mutate(rep = unlist(lapply(1:500, rep, 297))) %>%
  mutate(response = rep("CNTLIVE_TPA", 500 * 297),
         var = sd^2,
         section = str_remove_all(subsection, "[:lower:]"),
         province = str_sub(section, end = -2))
...
post_strat_voln_rs_df <- bind_rows(post_strat_voln_rs) %>%
  mutate(subsection = rep(names(subsection_list), 500)) %>%
  mutate(rep = unlist(lapply(1:500, rep, 297))) %>%
  mutate(response = rep("VOLNLIVE_TPA", 500 * 297),
         var = sd^2,
         section = str_remove_all(subsection, "[:lower:]"),
         province = str_sub(section, end = -2))
...
basal <- data.frame()
for(i in 1:500) {
  basal[i,1:297] <- eblupFH(ba_l[[i]]$ps_est ~ ba_l[[i]]$mean, ba_l[[i]]$var)$eblup
}
...
biolive <- data.frame()
for(i in 1:500) {
  biolive[i,1:297] <- eblupFH(bio_l[[i]]$ps_est ~ bio_l[[i]]$mean, bio_l[[i]]$var)$eblup
}
...
cnt <- data.frame()
for(i in 1:500) {
  cnt[i,1:297] <- eblupFH(cnt_l[[i]]$ps_est ~ cnt_l[[i]]$mean, cnt_l[[i]]$var)$eblup
}
...
voln <- data.frame()
for(i in 1:500) {
  voln[i,1:297] <- eblupFH(voln_l[[i]]$ps_est ~ voln_l[[i]]$mean, voln_l[[i]]$var)$eblup
}
