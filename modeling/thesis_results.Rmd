...
intwest_no0 <- intwest %>%
  ...
  filter(!(subsection %in% c("342Fi", "331Kj", "342Dh", "341Dc"))) # tiny init var, error in area-level
```
...
final_results_long <- final_results_long %>%
  dplyr::select(-ps_x, -sd_dirps, -se_freq_area)
```

```{r}
write.csv(final_results_long, "final_results_long.csv")
saveRDS(final_results_long, file = "final_results_long.rds")
```
